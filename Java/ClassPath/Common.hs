...
-- | mapM on forest
mapFM :: Monad m => (t -> m a) -> [Tree t] -> m [Tree a]
mapFM fn = mapM (mapTM fn)

-- | mapM on tree
mapTM ::  Monad m => (t -> m a) -> Tree t -> m (Tree a)
mapTM fn (Directory dir forest) = Directory dir `fmap` mapFM fn forest
mapTM fn (File a)               = File `fmap` fn a

mapFMF ::  Monad m => (FilePath -> t -> m a) -> [Tree t] -> m [Tree a]
mapFMF fn = mapM (mapTMF fn)

mapTMF ::  Monad m => (FilePath -> t -> m a) -> Tree t -> m (Tree a)
mapTMF fn = go ""
  where
